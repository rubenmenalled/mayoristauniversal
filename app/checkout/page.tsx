...
export default function CheckoutPage() {
  ...
  if (items.length === 0 && !confirmado) {
    return (
      <div style={{ minHeight: '100vh', background: 'linear-gradient(180deg,#7B1450,#8E1A5E)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 16 }}>
        <div style={{ fontSize: 60 }}>🛒</div>
        <div style={{ color: '#FFFFFF', fontWeight: 900, fontSize: 20 }}>Tu carrito está vacío</div>
        <button onClick={() => router.push('/')} style={{ background: GOLD, border: 'none', borderRadius: 10, padding: '12px 28px', color: '#7B1450', fontWeight: 900, cursor: 'pointer' }}>
          Ver productos
        </button>
      </div>
    )
  ...
  if (confirmado) {
    return (
      <div style={{ minHeight: '100vh', background: 'linear-gradient(180deg,#7B1450,#8E1A5E)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(180deg,#7B1450,#8E1A5E)' }}>
      {/* Header */}
      <div style={{ background: 'rgba(142,26,94,0.97)', borderBottom: '1px solid rgba(212,175,55,0.2)', padding: '16px 24px', position: 'sticky', top: 0, zIndex: 50, backdropFilter: 'blur(16px)' }}>
        ...
        <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(212,175,55,0.15)', borderRadius: 16, padding: 24 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 20 }}>
            {items.map(item => (
              <div key={item.id} style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                <div style={{ width: 44, height: 44, borderRadius: 8, overflow: 'hidden', flexShrink: 0, background: '#8E1A5E', position: 'relative' }}>
                  {item.image ? (
          ...
          <button
            ...
            style={{
              ...
              color: '#7B1450', fontWeight: 900, fontSize: 15,

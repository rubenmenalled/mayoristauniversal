...
export default function CategoriaPage() {
  ...
  if (esAdultos && !ageVerified) {
    return (
      <div style={{ minHeight: '100vh', background: 'linear-gradient(180deg, #4B5563 0%, #374151 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      ...
      {diasAmigo !== null && diasAmigo > 0 && (
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '14px 16px 0' }}>
          <div style={{
            ...
            background: 'linear-gradient(135deg,#4B5563 0%,#14B8A6 55%,#22C55E 100%)',
                  ...
                  <div style={{ padding: 8 }}>
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        const esU = sub === 'POP IT' || sub === 'LENCERIA POR BULTO' || sub === 'TOYS.AR' || ['IMPORTADORA MAX','IMPORTADORA COMEX','IMPORTADORA TREN'].includes(cat)
            ...
            <div onClick={e => e.stopPropagation()} style={{ position: 'relative', width: '100%', maxWidth: isMobile ? '100%' : 860, height: isMobile ? 'calc(100vh - 32px)' : 'calc(100vh - 48px)', maxHeight: isMobile ? '100%' : 640, background: '#FFFFFF', borderRadius: isMobile ? 16 : 20, overflow: 'hidden', border: '1px solid rgba(255,106,61,0.3)', display: 'flex', flexDirection: isMobile ? 'column' : 'row' }}>
              ...
              {!zoom && <div style={{ flex: 1, overflow: 'auto', padding: isMobile ? '12px 14px 14px' : '18px 18px 18px', display: 'flex', flexDirection: 'column', minWidth: 0 }}>
                ...
                <div style={{ marginTop: 'auto', paddingTop: 8 }}>
                  {(() => {
                    // POP IT guardan precio UNITARIO; el resto guarda precio del pack
                    ...
                    const esU = (lightbox.subcategory ?? '').toUpperCase() === 'POP IT' || esBulto || esDocena

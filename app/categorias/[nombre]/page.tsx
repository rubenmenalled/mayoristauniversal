                  ...
                  <div style={{ padding: 8 }}>
                    ...
                    {(() => {
                      ...
                      if (p.descripcion?.startsWith('PRECIO POR')) {
                        ...
                        const esCot = (p.category ?? '').toUpperCase() === 'COTILLON' || (p.subcategory ?? '').toUpperCase() === 'POP IT'
                    ...
                    <button
                      style={{ width: '100%', marginTop: 10, padding: '7px', borderRadius: 8, border: 'none', cursor: 'pointer', background: 'linear-gradient(135deg,#D4AF37,#F0C030)', color: '#5C3A00', fontSize: 13, fontWeight: 900, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4, WebkitTapHighlightColor: 'transparent' }}
                      onClick={() => {
              ...
              {!zoom && <div style={{ flex: 1, overflow: 'auto', padding: isMobile ? '12px 14px 14px' : '18px 18px 18px', display: 'flex', flexDirection: 'column', minWidth: 0 }}>
                ...
                <div style={{ marginTop: 'auto', paddingTop: 8 }}>
                  {(() => {
                    ...
                    const esU = (lightbox.category ?? '').toUpperCase() === 'COTILLON' || (lightbox.subcategory ?? '').toUpperCase() === 'POP IT'
                      ...
                      <>
                        {lightbox.minOrder > 1 && (
                          <div style={{ background: '#FFFDE7', border: '1.5px solid #F59E0B', borderRadius: 8, padding: '8px 12px', marginBottom: 8 }}>
                            ...
                            <div style={{ color: '#B45309', fontSize: 13, fontWeight: 700, marginTop: 2 }}>{lightbox.minOrder === 12 ? 'LA DOCENA' : `PACK X ${lightbox.minOrder}`}: ${packTotal.toLocaleString('es-AR')}</div>
                          </div>
                        )}
                        <div style={{ color: '#D4AF37', fontWeight: 900, fontSize: isMobile ? 26 : 22 }}>
                          ...
                          {lightbox.minOrder > 1 && <span style={{ fontSize: 13, fontWeight: 600, color: '#92400E', marginLeft: 6 }}>{lightbox.minOrder === 12 ? 'la docena' : `el pack x ${lightbox.minOrder}`}</span>}
                        </div>
                      </>
                    )
                  })()}
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
                  ...
                  <motion.button whileTap={{ scale: 0.97 }}
                    ...
                    style={{ flex: 2, background: 'linear-gradient(135deg,#D4AF37,#F0C030)', border: 'none', borderRadius: 10, padding: isMobile ? '14px 6px' : '10px 6px', color: '#5C3A00', fontWeight: 900, cursor: 'pointer', fontSize: isMobile ? 16 : 14, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5 }}>

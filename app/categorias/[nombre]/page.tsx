              ...
              {productosFiltrados.map((p, i) => (
                <motion.div key={p.id}
                  ...
                  <div style={{ padding: 10 }}>
                    ...
                    <Stars n={p.rating} />
                    {(() => {
                      const esPaquete = p.category?.toUpperCase() === 'ACCESORIOS DE PELO' || (p.category?.toUpperCase() === 'MARROQUINERIA' && p.subcategory?.toUpperCase() !== 'MOCHILAS-CARTERAS')
                    ...
                    <div style={{ marginTop: 6 }}>
                      <div style={{ color: '#CBD5E1', fontSize: 10, marginTop: 2 }}>
                        {(p.category?.toUpperCase() === 'ACCESORIOS DE PELO' || (p.category?.toUpperCase() === 'MARROQUINERIA' && p.subcategory?.toUpperCase() !== 'MOCHILAS-CARTERAS')) ? 'Precio x docena:' : 'Mayorista:'} <span style={{ color: '#D4AF37', fontWeight: 900, fontSize: 14 }}>${p.wholesalePrice.toLocaleString('es-AR')}</span>
                      </div>
                    </div>
                    <button
          ...
          <motion.div
            ...
            <div onClick={e => e.stopPropagation()} style={{ position: 'relative', width: '100%', maxWidth: zoom ? '100%' : 500, maxHeight: zoom ? '100vh' : 'calc(100vh - 40px)', background: '#0a1628', borderRadius: zoom ? 0 : 20, overflow: zoom ? 'hidden' : 'auto', border: zoom ? 'none' : '1px solid rgba(212,175,55,0.3)', transition: 'all 0.25s ease' }}>
              ...
              {!zoom && <div style={{ padding: '16px 20px 20px' }}>
                ...
                {(lightbox.category?.toUpperCase() === 'ACCESORIOS DE PELO' || (lightbox.category?.toUpperCase() === 'MARROQUINERIA' && lightbox.subcategory?.toUpperCase() !== 'MOCHILAS-CARTERAS')) && (
                ...
                {lightbox.category?.toUpperCase() !== 'ACCESORIOS DE PELO' && !(lightbox.category?.toUpperCase() === 'MARROQUINERIA' && lightbox.subcategory?.toUpperCase() !== 'MOCHILAS-CARTERAS') && <div style={{ marginBottom: 12 }} />}

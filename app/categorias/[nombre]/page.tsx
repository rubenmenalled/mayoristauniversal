                  ...
                  <div style={{ padding: 8 }}>
                    ...
                    {(() => {
                      ...
                      if (p.descripcion?.startsWith('PRECIO POR')) {
                        ...
                        const esCot = (p.category ?? '').toUpperCase() === 'COTILLON MAGIC KINGDOM' || (p.subcategory ?? '').toUpperCase() === 'POP IT' || (p.subcategory ?? '').toUpperCase() === 'INFAC-TEC' || (p.subcategory ?? '').toUpperCase() === 'TOYS.AR' || (['IMPORTADORA MAX','IMPORTADORA COMEX'].includes((p.category ?? '').toUpperCase()))
                        if (esDocena) {
                      ...
                      return titulo ? (
                        <>
                          <div style={{ background: '#FFFDE7', border: '1.5px solid #F59E0B', borderRadius: 6, padding: '4px 8px', marginBottom: extraInfo ? 2 : 4 }}>
                            ...
                            {(p.subcategory ?? '').toUpperCase() !== 'TOYS.AR' && !(['IMPORTADORA MAX','IMPORTADORA COMEX'].includes((p.category ?? '').toUpperCase())) && <span style={{ color: '#111', fontSize: 10, fontWeight: 900, letterSpacing: '0.02em' }}>{titulo}</span>}
                            <div style={{ color: '#C2410C', fontSize: 13, fontWeight: 900, marginTop: 2 }}>{(() => {
                              const esDoc = (p.subcategory ?? '').toUpperCase() === 'LENCERIA POR BULTO'
                              const esBulk = (p.category ?? '').toUpperCase() === 'COTILLON MAGIC KINGDOM' || (p.subcategory ?? '').toUpperCase() === 'POP IT' || (p.subcategory ?? '').toUpperCase() === 'INFAC-TEC' || (p.subcategory ?? '').toUpperCase() === 'TOYS.AR' || (['IMPORTADORA MAX','IMPORTADORA COMEX'].includes((p.category ?? '').toUpperCase()))
                              const total = (esBulk || esDoc) ? p.wholesalePrice * p.minOrder : p.wholesalePrice
                              const label = esDoc ? `EL BULTO (${p.minOrder} DOCENAS)` : ((p.subcategory ?? '').toUpperCase() === 'TOYS.AR' || (['IMPORTADORA MAX','IMPORTADORA COMEX'].includes((p.category ?? '').toUpperCase()))) ? `EL BULTO X${p.minOrder}` : p.minOrder === 12 ? 'LA DOCENA' : `PACK X ${p.minOrder}`
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        const esU = cat === 'COTILLON MAGIC KINGDOM' || sub === 'POP IT' || sub === 'LENCERIA POR BULTO' || sub === 'TOYS.AR' || ['IMPORTADORA MAX','IMPORTADORA COMEX'].includes(cat)
              ...
              {!zoom && <div style={{ flex: 1, overflow: 'auto', padding: isMobile ? '12px 14px 14px' : '18px 18px 18px', display: 'flex', flexDirection: 'column', minWidth: 0 }}>
                ...
                <div style={{ marginTop: 'auto', paddingTop: 8 }}>
                  {(() => {
                    ...
                    const esBulto = (lightbox.subcategory ?? '').toUpperCase() === 'TOYS.AR' || (['IMPORTADORA MAX','IMPORTADORA COMEX'].includes((lightbox.category ?? '').toUpperCase()))

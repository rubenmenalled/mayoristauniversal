                ...
                <motion.div key={p.id}
                  ...
                  <div style={{ padding: 10 }}>
                    ...
                    {(() => {
                      ...
                      const mostrarBadge = esCat && !['MOCHILAS-CARTERAS','MOCHILAS'].includes(p.subcategory?.toUpperCase() ?? '')
                    ...
                    <div style={{ marginTop: 6 }}>
                      <div style={{ color: '#CBD5E1', fontSize: 10, marginTop: 2 }}>
                        {(p.category?.toUpperCase() === 'ACCESORIOS DE PELO' || (p.category?.toUpperCase() === 'MARROQUINERIA' && !['MOCHILAS-CARTERAS','MOCHILAS'].includes(p.subcategory?.toUpperCase() ?? ''))) ? 'Precio x docena:' : 'Mayorista:'} <span style={{ color: '#D4AF37', fontWeight: 900, fontSize: 14 }}>${p.wholesalePrice.toLocaleString('es-AR')}</span>
                      </div>
                    </div>
                    <button
              ...
              {!zoom && <div style={{ padding: '16px 20px 20px' }}>
                ...
                {(lightbox.category?.toUpperCase() === 'ACCESORIOS DE PELO' || lightbox.category?.toUpperCase() === 'MARROQUINERIA') && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 10 }}>
                    {!['MOCHILAS-CARTERAS','MOCHILAS'].includes(lightbox.subcategory?.toUpperCase() ?? '') && (
                ...
                {lightbox.category?.toUpperCase() !== 'ACCESORIOS DE PELO' && !(lightbox.category?.toUpperCase() === 'MARROQUINERIA' && !['MOCHILAS-CARTERAS','MOCHILAS'].includes(lightbox.subcategory?.toUpperCase() ?? '')) && <div style={{ marginBottom: 12 }} />}

...
export default function CategoriaPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(180deg, #7B1450 0%, #8E1A5E 100%)', paddingTop: 38 }}>
      {/* Header — debajo del AnnouncementBar (38px fijo) */}
      <div style={{
        background: 'rgba(142,26,94,0.97)', borderBottom: '1px solid rgba(212,175,55,0.2)',
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <a href="/catalogo" style={{
            ...
            color: '#7B1450', fontWeight: 900, fontSize: 12,
          ...
          <button onClick={() => setCartOpen(true)} style={{ position: 'relative', background: 'none', border: 'none', cursor: 'pointer', color: '#FFFFFF', padding: 6 }}>
            ...
            {count > 0 && <span style={{ position: 'absolute', top: 0, right: 0, background: '#D4AF37', color: '#7B1450', fontSize: 10, width: 16, height: 16, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900 }}>{count}</span>}
          </button>
        </div>
      </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 16 }}>
            ...
            <motion.div
              ...
              <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                ...
                <span style={{ color: '#7B1450', fontWeight: 900, fontSize: 16, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Ver todos</span>
                <span style={{ color: '#7B1450', fontSize: 12, fontWeight: 700 }}>{productos.length} productos</span>
              </div>
            </motion.div>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '32px 16px' }}>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: 80, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(212,175,55,0.15)', borderRadius: 20 }}>
            ...
            {subActiva && (
              <button onClick={() => setSubActiva('')}
                style={{ marginTop: 16, background: 'linear-gradient(135deg,#D4AF37,#F0C030)', border: 'none', borderRadius: 10, padding: '10px 24px', color: '#7B1450', fontWeight: 900, cursor: 'pointer' }}>
                Ver todos
              </button>
            )}
          </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 14 }}>
              {productosFiltrados.map((p, i) => (
                <motion.div key={p.id}
                  ...
                  whileHover={{ y: -4 }}>
                  <div style={{ position: 'relative', height: 150, background: '#8E1A5E', overflow: 'hidden', cursor: p.image ? 'zoom-in' : 'default' }}
                    ...
                    <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(142,26,94,0.7), transparent)' }} />
                    {(p.discount ?? 0) > 0 && (
                  ...
                  <div style={{ padding: 10 }}>
                    ...
                    <motion.button
                      style={{ width: '100%', marginTop: 10, padding: '7px', borderRadius: 8, border: 'none', cursor: 'pointer', background: 'linear-gradient(135deg,#D4AF37,#F0C030)', color: '#7B1450', fontSize: 11, fontWeight: 900, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}
                      whileTap={{ scale: 0.97 }}
      ...
      {lightbox && (
        <motion.div
          ...
          <div onClick={e => e.stopPropagation()} style={{ position: 'relative', width: '100%', maxWidth: 500, background: '#0a1628', borderRadius: 20, overflow: 'hidden', border: '1px solid rgba(212,175,55,0.3)' }}>
            {/* Foto grande */}
            <div style={{ position: 'relative', width: '100%', aspectRatio: '1', background: '#8E1A5E' }}>
            ...
            <div style={{ padding: '16px 20px 20px' }}>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                <motion.button whileTap={{ scale: 0.97 }}
                  ...
                  style={{ flex: 2, background: 'linear-gradient(135deg,#D4AF37,#F0C030)', border: 'none', borderRadius: 10, padding: '10px', color: '#7B1450', fontWeight: 900, cursor: 'pointer', fontSize: 13, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>

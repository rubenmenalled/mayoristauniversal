    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(180deg, #0B1E3F 0%, #13294f 100%)', paddingTop: 38 }}>
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <form onSubmit={handleSearch} style={{ flex: 1, display: 'flex', gap: 0, borderRadius: 10, overflow: 'hidden', border: '1px solid rgba(255,106,61,0.3)' }}>
            <input
            ...
            />
            <button type="submit" style={{ background: 'linear-gradient(135deg,#FF6A3D,#FF8A63)', border: 'none', padding: '10px 20px', cursor: 'pointer' }}>
          ...
          <>
            <div style={{ color: 'rgba(255,255,255,0.7)', fontSize: 13, marginBottom: 24 }}>
              {productos.length === 0
                ...
                : <>{productos.length} resultado{productos.length !== 1 ? 's' : ''} para <span style={{ color: '#FF6A3D', fontWeight: 700 }}>"{q}"</span></>
              }
            </div>
            ...
            {productos.length === 0 ? (
              <div style={{ textAlign: 'center', padding: 60, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,106,61,0.15)', borderRadius: 20 }}>
                ...
                <button onClick={() => router.push('/catalogo')}
                  style={{ marginTop: 20, background: 'linear-gradient(135deg,#FF6A3D,#FF8A63)', border: 'none', borderRadius: 10, padding: '12px 28px', color: '#FFFFFF', fontWeight: 900, cursor: 'pointer' }}>
                  Ver catálogos
                </button>
              </div>
            ) : (
                    ...
                    <div style={{ padding: 8 }}>
                      ...
                      {(() => {
                        ...
                        if (p.descripcion?.startsWith('PRECIO POR')) {
                          ...
                          const esCot = (p.category ?? '').toUpperCase() === 'COTILLON MAGIC KINGDOM' || (p.subcategory ?? '').toUpperCase() === 'POP IT' || (['IMPORTADORA MAX','IMPORTADORA COMEX','IMPORTADORA TRENDY'].includes((p.category ?? '').toUpperCase()))
                          if (esDocena) {
                          ...
                          <>
                            <div style={{ background: '#FFFDE7', border: '1.5px solid #F59E0B', borderRadius: 6, padding: '4px 8px', marginBottom: extraInfo ? 2 : 4 }}>
                              ...
                              <div style={{ color: '#C2410C', fontSize: 13, fontWeight: 900, marginTop: 2 }}>{(() => {
                                ...
                                const esBulk = (p.category ?? '').toUpperCase() === 'COTILLON MAGIC KINGDOM' || (p.subcategory ?? '').toUpperCase() === 'POP IT' || (['IMPORTADORA MAX','IMPORTADORA COMEX','IMPORTADORA TRENDY'].includes((p.category ?? '').toUpperCase()))
                                const total = (esBulk || esDoc) ? p.wholesalePrice * p.minOrder : p.wholesalePrice
                                const label = esDoc ? `EL BULTO (${p.minOrder} DOCENAS)` : (['IMPORTADORA MAX','IMPORTADORA COMEX','IMPORTADORA TRENDY'].includes((p.category ?? '').toUpperCase())) ? `EL BULTO X${p.minOrder}` : p.minOrder === 12 ? 'LA DOCENA' : `PACK X ${p.minOrder}`
                        ...
                        return (
                          <div style={{ marginTop: 6 }}>
                            <div style={{ color: '#6B7280', fontSize: 10, marginTop: 2 }}>
                              Mayorista: <span style={{ color: '#FF6A3D', fontWeight: 900, fontSize: 14 }}>${p.wholesalePrice.toLocaleString('es-AR')}</span>
                            </div>
                          </div>
                        )
                      })()}
                      <motion.button
                        ...
                        style={{ width: '100%', marginTop: 10, padding: '7px', borderRadius: 8, border: 'none', cursor: 'pointer', background: 'linear-gradient(135deg,#FF6A3D,#FF8A63)', color: '#0D2C54', fontSize: 12, fontWeight: 900, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}
                        whileTap={{ scale: 0.97 }}
                        onClick={() => {
                          const cat = (p.category ?? '').toUpperCase()
                          const sub = (p.subcategory ?? '').toUpperCase()
                          const esU = cat === 'COTILLON MAGIC KINGDOM' || sub === 'POP IT' || sub === 'LENCERIA POR BULTO' || ['IMPORTADORA MAX','IMPORTADORA COMEX','IMPORTADORA TRENDY'].includes(cat)

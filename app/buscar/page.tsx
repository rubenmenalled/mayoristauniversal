                    ...
                    <div style={{ padding: 8 }}>
                      ...
                      {(() => {
                        ...
                        if (p.descripcion?.startsWith('PRECIO POR')) {
                          ...
                          const esCot = (p.category ?? '').toUpperCase() === 'COTILLON MAGIC KINGDOM' || (p.subcategory ?? '').toUpperCase() === 'POP IT' || (['IMPORTADORA MAX','IMPORTADORA COMEX'].includes((p.category ?? '').toUpperCase()))
                          if (esDocena) {
                          ...
                          <>
                            <div style={{ background: '#FFFDE7', border: '1.5px solid #F59E0B', borderRadius: 6, padding: '4px 8px', marginBottom: extraInfo ? 2 : 4 }}>
                              ...
                              <div style={{ color: '#C2410C', fontSize: 13, fontWeight: 900, marginTop: 2 }}>{(() => {
                                ...
                                const esBulk = (p.category ?? '').toUpperCase() === 'COTILLON MAGIC KINGDOM' || (p.subcategory ?? '').toUpperCase() === 'POP IT' || (['IMPORTADORA MAX','IMPORTADORA COMEX'].includes((p.category ?? '').toUpperCase()))
                                const total = (esBulk || esDoc) ? p.wholesalePrice * p.minOrder : p.wholesalePrice
                                const label = esDoc ? `EL BULTO (${p.minOrder} DOCENAS)` : (['IMPORTADORA MAX','IMPORTADORA COMEX'].includes((p.category ?? '').toUpperCase())) ? `EL BULTO X${p.minOrder}` : p.minOrder === 12 ? 'LA DOCENA' : `PACK X ${p.minOrder}`
                      ...
                      <motion.button
                        ...
                        onClick={() => {
                          ...
                          const esU = cat === 'COTILLON MAGIC KINGDOM' || sub === 'POP IT' || sub === 'LENCERIA POR BULTO' || ['IMPORTADORA MAX','IMPORTADORA COMEX'].includes(cat)

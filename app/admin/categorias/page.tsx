...
const EMPTY: Categoria = { nombre: '', emoji: '📦', descripcion: '', imagen: '', cantidad: 0 }
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg,#030D1E 0%,#071633 100%)' }}>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '24px 16px' }}>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
                <h2 style={{ color: '#fff', margin: 0, fontWeight: 900 }}>
                  {form.id ? 'Editar Rubro' : 'Nuevo Rubro'}
                </h2>
              ...
              <div style={{ display: 'grid', gap: 14 }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                    {EMOJIS.map(e => (
                      <button key={e} onClick={() => setForm(f => ({ ...f, emoji: e }))}
                        style={{
                          fontSize: 22, padding: '6px 8px', borderRadius: 8, cursor: 'pointer',
                          background: form.emoji === e ? 'rgba(212,175,55,0.3)' : 'rgba(255,255,255,0.07)',
                          border: form.emoji === e ? '2px solid #D4AF37' : '1px solid transparent',
                        }}>{e}</button>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <input value={form.descripcion} onChange={e => setForm(f => ({ ...f, descripcion: e.target.value }))}
                    style={{ width: '100%', background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(212,175,55,0.25)', borderRadius: 8, padding: '10px 12px', color: '#fff', fontSize: 14, outline: 'none', boxSizing: 'border-box' }}
                    placeholder="Ej: Gadgets y electrónica" />
                </div>

                <div>
                  <label style={{ color: '#D4AF37', fontSize: 11, fontWeight: 700, display: 'block', marginBottom: 4 }}>CANTIDAD DE PRODUCTOS</label>
                  <input type="number" value={form.cantidad} onChange={e => setForm(f => ({ ...f, cantidad: Number(e.target.value) }))}
                    style={{ width: '100%', background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(212,175,55,0.25)', borderRadius: 8, padding: '10px 12px', color: '#fff', fontSize: 14, outline: 'none', boxSizing: 'border-box' }} />
                </div>
                ...
                <div>
                  ...
                  {form.imagen && (
                    <img src={form.imagen} alt="" style={{ width: '100%', height: 120, objectFit: 'cover', borderRadius: 8, marginBottom: 8 }} />
                  )}
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 24 }}>
                <button onClick={() => setShowForm(false)}
                  style={{ flex: 1, background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, padding: 12, color: '#ccc', fontWeight: 700, cursor: 'pointer' }}>
                  Cancelar
                </button>
                <button onClick={handleSave} disabled={saving}
                  style={{ flex: 2, background: GOLD, border: 'none', borderRadius: 10, padding: 12, color: '#030D1E', fontWeight: 900, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, opacity: saving ? 0.7 : 1 }}>
                  <Check size={16} />
                  {saving ? 'Guardando...' : 'GUARDAR RUBRO'}
                </button>
              </div>
            </div>
          </div>
        ...
        {/* Lista */}
        {loading ? (
          <div style={{ color: '#7a8a9a', textAlign: 'center', padding: 60 }}>Cargando rubros...</div>
        ) : categorias.length === 0 ? (
          <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(212,175,55,0.15)', borderRadius: 16, padding: 60, textAlign: 'center' }}>
            ...
            <div style={{ color: '#fff', fontWeight: 900, fontSize: 18, marginBottom: 8 }}>No hay rubros todavía</div>
            <div style={{ color: '#7a8a9a', fontSize: 13 }}>Hacé clic en "Agregar Rubro" para empezar</div>
          </div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px,1fr))', gap: 14 }}>
            {categorias.map(c => (
              <div key={c.id} style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(212,175,55,0.15)', borderRadius: 14, overflow: 'hidden' }}>
                {c.imagen && <img src={c.imagen} alt={c.nombre} style={{ width: '100%', height: 100, objectFit: 'cover' }} />}
                <div style={{ padding: '12px 14px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                    <span style={{ fontSize: 20 }}>{c.emoji}</span>
                    <span style={{ color: '#fff', fontWeight: 900 }}>{c.nombre}</span>
                  </div>
                  <div style={{ color: '#7a8a9a', fontSize: 12, marginBottom: 12 }}>{c.descripcion}</div>
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button onClick={() => { setForm(c); setShowForm(true) }}
                      style={{ flex: 1, background: 'rgba(212,175,55,0.15)', border: '1px solid rgba(212,175,55,0.3)', borderRadius: 8, padding: '7px', color: '#D4AF37', cursor: 'pointer', fontSize: 12, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
                      <Pencil size={12} /> Editar
                    </button>
                    <button onClick={() => handleDelete(c.id!)}
                      style={{ flex: 1, background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 8, padding: '7px', color: '#f87171', cursor: 'pointer', fontSize: 12, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}>

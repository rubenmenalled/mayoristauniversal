...
export default function CategoriasAdmin() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0F172A' }}>
      {/* Header */}
      <div style={{ background: '#1E293B', borderBottom: '1px solid rgba(255,106,61,0.3)', padding: '16px 24px', display: 'flex', alignItems: 'center', gap: 16 }}>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '24px 16px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 20 }}>
          <button onClick={() => { setForm(EMPTY_CAT); setShowForm(true) }}
            style={{ background: GOLD, border: 'none', borderRadius: 10, padding: '10px 20px', color: '#FFFFFF', fontWeight: 900, fontSize: 14, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        {showForm && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 16 }}>
            <div style={{ background: '#0a1628', border: '1px solid rgba(255,106,61,0.3)', borderRadius: 20, padding: 28, width: '100%', maxWidth: 480, maxHeight: '90vh', overflowY: 'auto' }}>
              ...
              <div style={{ display: 'grid', gap: 14 }}>
                <div>
                  <label style={{ color: '#FF6A3D', fontSize: 11, fontWeight: 700, display: 'block', marginBottom: 4 }}>NOMBRE</label>
                  <input value={form.nombre} onChange={e => setForm(f => ({ ...f, nombre: e.target.value }))}
                    style={{ width: '100%', background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,106,61,0.25)', borderRadius: 8, padding: '10px 12px', color: '#FFFFFF', fontSize: 14, outline: 'none', boxSizing: 'border-box' }}
                    placeholder="Ej: Juguetes" />
                </div>
                <div>
                  <label style={{ color: '#FF6A3D', fontSize: 11, fontWeight: 700, display: 'block', marginBottom: 8 }}>EMOJI</label>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                    {EMOJIS.map(e => (
                      <button key={e} onClick={() => setForm(f => ({ ...f, emoji: e }))}
                        style={{ fontSize: 22, padding: '6px 8px', borderRadius: 8, cursor: 'pointer', background: form.emoji === e ? 'rgba(255,106,61,0.3)' : 'rgba(255,255,255,0.07)', border: form.emoji === e ? '2px solid #FF6A3D' : '1px solid transparent' }}>{e}</button>
                    ))}
                  </div>
                </div>
                <div>
                  <label style={{ color: '#FF6A3D', fontSize: 11, fontWeight: 700, display: 'block', marginBottom: 4 }}>DESCRIPCIÓN</label>
                  <input value={form.descripcion} onChange={e => setForm(f => ({ ...f, descripcion: e.target.value }))}
                    style={{ width: '100%', background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,106,61,0.25)', borderRadius: 8, padding: '10px 12px', color: '#FFFFFF', fontSize: 14, outline: 'none', boxSizing: 'border-box' }}
                    placeholder="Ej: Muñecos y peluches" />
                </div>
                <div>
                  <label style={{ color: '#FF6A3D', fontSize: 11, fontWeight: 700, display: 'block', marginBottom: 4 }}>FOTO DEL RUBRO</label>
                  ...
                  <button type="button" onClick={() => fileRef.current?.click()} disabled={uploadingImg}
                    style={{ width: '100%', background: 'rgba(255,255,255,0.07)', border: '1px dashed rgba(255,106,61,0.4)', borderRadius: 8, padding: '12px', color: '#FF6A3D', fontSize: 13, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 24 }}>
                ...
                <button onClick={handleSave} disabled={saving} style={{ flex: 2, background: GOLD, border: 'none', borderRadius: 10, padding: 12, color: '#FFFFFF', fontWeight: 900, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, opacity: saving ? 0.7 : 1 }}>
        ...
        {showSubForm && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 16 }}>
            <div style={{ background: '#0a1628', border: '1px solid rgba(255,106,61,0.3)', borderRadius: 20, padding: 28, width: '100%', maxWidth: 400 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                    {EMOJIS.map(e => (
                      <button key={e} onClick={() => setSubForm(f => ({ ...f, emoji: e }))}
                        style={{ fontSize: 20, padding: '5px 7px', borderRadius: 8, cursor: 'pointer', background: subForm.emoji === e ? 'rgba(255,106,61,0.3)' : 'rgba(255,255,255,0.07)', border: subForm.emoji === e ? '2px solid #FF6A3D' : '1px solid transparent' }}>{e}</button>
                    ))}
                  </div>
                </div>
              </div>
              <div style={{ display: 'flex', gap: 10, marginTop: 24 }}>
                ...
                <button onClick={handleSaveSub} disabled={savingSub} style={{ flex: 2, background: GOLD, border: 'none', borderRadius: 10, padding: 12, color: '#FFFFFF', fontWeight: 900, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, opacity: savingSub ? 0.7 : 1 }}>
        ...
        {loading ? (
          ...
          <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,106,61,0.15)', borderRadius: 16, padding: 60, textAlign: 'center' }}>
          ...
          <div style={{ display: 'grid', gap: 12 }}>
            {categorias.map(c => (
              <div key={c.id} style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,106,61,0.15)', borderRadius: 14, overflow: 'hidden' }}>
                {/* Fila categoría */}
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px' }}>
                  ...
                  <span style={{ color: '#FFFFFF', fontWeight: 900, fontSize: 15, flex: 1 }}>{c.nombre}</span>
                  <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                    ...
                    <button onClick={() => { setForm(c); setShowForm(true) }}
                      style={{ background: 'rgba(255,106,61,0.15)', border: '1px solid rgba(255,106,61,0.3)', borderRadius: 8, padding: '7px 12px', color: '#FF6A3D', cursor: 'pointer', fontSize: 12, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 4 }}>
                ...
                {expandedCat === c.id && (
                  <div style={{ borderTop: '1px solid rgba(255,106,61,0.1)', background: 'rgba(0,0,0,0.2)', padding: '12px 16px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                      ...
                      <button
                        ...
                        style={{ background: GOLD, border: 'none', borderRadius: 8, padding: '6px 12px', color: '#FFFFFF', fontWeight: 900, fontSize: 12, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>
                        ...
                        {subcategorias[c.id!].map(sub => (
                          <div key={sub.id} style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'rgba(255,255,255,0.04)', borderRadius: 8, padding: '8px 12px' }}>
                            ...
                            <button onClick={() => { setSubForm(sub); setShowSubForm(true) }}
                              style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#FF6A3D', padding: 4 }}>
                              <Pencil size={13} />
                            </button>

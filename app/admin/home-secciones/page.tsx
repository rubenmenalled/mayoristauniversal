  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 8 }}>
        ...
        {ids.map(id => (
          <span key={id} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'rgba(255,106,61,0.15)', border: '1px solid rgba(255,106,61,0.35)', borderRadius: 6, padding: '4px 8px', color: '#fff', fontSize: 12 }}>
            {nombres[id] || `#${id}`}
      ...
      <input value={term} onChange={e => setTerm(e.target.value)} placeholder="🔍 Buscar producto por nombre o código para agregar..."
        style={{ width: '100%', boxSizing: 'border-box', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,106,61,0.3)', borderRadius: 8, padding: '9px 12px', color: '#fff', fontSize: 13 }} />
          ...
          {resultados.map(p => (
            <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,0.04)', borderRadius: 6, padding: '5px 8px' }}>
              ...
              <button onClick={() => agregar(p)} disabled={ids.includes(String(p.id))}
                style={{ background: ids.includes(String(p.id)) ? 'rgba(255,255,255,0.1)' : 'linear-gradient(135deg,#FF6A3D,#FF8A63)', border: 'none', borderRadius: 6, padding: '5px 10px', color: ids.includes(String(p.id)) ? '#7a8a9a' : '#0F3460', fontWeight: 800, fontSize: 12, cursor: 'pointer' }}>
...
export default function HomeSeccionesPage() {
  ...
  const ModeBtn = ({ active, onClick, children }: any) => (
    <button onClick={onClick} style={{ borderRadius: 8, padding: '7px 12px', fontSize: 13, fontWeight: 800, cursor: 'pointer',
      border: active ? '1px solid #FF6A3D' : '1px solid rgba(255,255,255,0.15)',
      background: active ? 'linear-gradient(135deg,#FF6A3D,#FF8A63)' : 'rgba(255,255,255,0.05)',
      ...
      <div style={{ maxWidth: 1000, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8, flexWrap: 'wrap', gap: 10 }}>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <button onClick={agregar} style={{ background: 'linear-gradient(135deg,#FF6A3D,#FF8A63)', border: 'none', borderRadius: 8, padding: '8px 16px', color: '#0F3460', fontWeight: 900, cursor: 'pointer' }}>+ Agregar fila</button>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,106,61,0.2)', borderRadius: 14, padding: 30, color: '#cbd5e1', fontSize: 14 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {filas.map(f => {
              ...
              return (
                <div key={f.id} style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,106,61,0.18)', borderRadius: 14, padding: 16 }}>
                  <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center', marginBottom: 10 }}>
                    ...
                    <input value={f.titulo} onChange={e => setLocal(f.id, { titulo: e.target.value })} placeholder="Título" style={{ flex: 1, minWidth: 180, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,106,61,0.3)', borderRadius: 8, padding: '10px 12px', color: '#fff', fontSize: 15, fontWeight: 700 }} />
                  ...
                  {modo === 'cat' && (
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 6, marginBottom: 12 }}>
                      {cats.map(cat => (
                        <label key={cat} style={{ color: '#cbd5e1', fontSize: 12, display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer', background: seleccionadas.includes(cat) ? 'rgba(255,106,61,0.12)' : 'transparent', border: `1px solid ${seleccionadas.includes(cat) ? 'rgba(255,106,61,0.35)' : 'rgba(255,255,255,0.08)'}`, borderRadius: 6, padding: '5px 8px' }}>

                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 8 }}>
                    {categorias.map((cat) => (
                      <button key={cat.id}
                        ...
                        onMouseEnter={e => { e.currentTarget.style.background = '#F0F5EE'; e.currentTarget.style.borderColor = '#E8DCC8' }}
              ...
              <div style={{ position: 'absolute', top: '100%', left: 0, zIndex: 500, background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.1)', borderRadius: 12, boxShadow: '0 12px 40px rgba(0,0,0,0.25)', width: 480, padding: '14px', animation: 'fadeInDown 0.15s ease' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12, paddingBottom: 10, borderBottom: '1px solid #F0F0F0' }}>
                  <button onClick={() => setOpenCat('__desktop__')}
                    style={{ background: 'none', border: 'none', color: '#E8DCC8', fontWeight: 700, fontSize: 13, cursor: 'pointer', padding: 0, display: 'flex', alignItems: 'center', gap: 4 }}>
                    ← Volver
                  </button>
                  <span style={{ color: '#CCC' }}>|</span>
                  <span style={{ fontSize: 20 }}>{categorias.find(c => c.nombre === openCat)?.emoji}</span>
                  <span style={{ color: '#E8DCC8', fontWeight: 900, fontSize: 14 }}>{openCat}</span>
                </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 6 }}>
                    {subsByCategory[openCat].map((sub) => (
                      <a key={sub} href={`/categorias/${encodeURIComponent(openCat)}?sub=${encodeURIComponent(sub)}`}
                        ...
                        onMouseEnter={e => { e.currentTarget.style.background = '#E8F0E4'; e.currentTarget.style.color = '#E8DCC8' }}
                        onMouseLeave={e => { e.currentTarget.style.background = '#F0F5EE'; e.currentTarget.style.color = '#444' }}>
                        <span style={{ color: '#E8DCC8', fontSize: 10, flexShrink: 0 }}>▸</span>{sub}
                      </a>
                    ))}
                  </div>
                )}
                <a href={`/categorias/${encodeURIComponent(openCat)}`}
                  onClick={() => setOpenCat(null)}
                  style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, marginTop: 12, padding: '10px', background: '#E8DCC8', borderRadius: 8, color: '#FFFFFF', fontWeight: 800, fontSize: 13, textDecoration: 'none' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px 10px', borderBottom: '1px solid #EEE', flexShrink: 0 }}>
                  {mobileSelectedCat ? (
                    <button onClick={() => setMobileSelectedCat(null)}
                      style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'none', border: 'none', color: '#E8DCC8', fontWeight: 700, fontSize: 14, cursor: 'pointer', padding: 0 }}>
                      ← Volver
                    </button>
                  ) : (
                  ...
                  {!mobileSelectedCat ? (
                    ...
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14, padding: '4px 0' }}>
                        ...
                        <span style={{ color: '#E8DCC8', fontWeight: 900, fontSize: 16 }}>{mobileSelectedCat}</span>
                      </div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 8 }}>
                          {subsByCategory[mobileSelectedCat].map((sub) => (
                            <a key={sub}
                              ...
                              <span style={{ color: '#E8DCC8', fontSize: 11, flexShrink: 0 }}>▸</span>{sub}
                            </a>
                          ))}
                        </div>
                      )}
                      <a href={`/categorias/${encodeURIComponent(mobileSelectedCat)}`}
                        onClick={() => { setMobileGridOpen(false); setMobileSelectedCat(null) }}
                        style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, marginTop: 14, padding: '13px', background: '#E8DCC8', borderRadius: 12, color: '#FFFFFF', fontWeight: 800, fontSize: 14, textDecoration: 'none' }}>
                        Ver todos los productos →
                      </a>
                    </div>
                  )}

...
export default function Header() {
  ...
  return (
    ...
    <header className={`fixed inset-x-0 z-50 transition-all duration-400 ${
      ...
      style={{ top: 38, background: scrolled ? 'rgba(248,245,238,0.97)' : 'rgba(248,245,238,0.92)', backdropFilter: 'blur(16px)' }}>
          ...
          <div className="lg:hidden ml-auto flex items-center gap-1.5">
            ...
            <a href={userName ? '/mi-cuenta' : '/login'} style={{ textDecoration: 'none', flexShrink: 0 }}>
              <div style={{
                ...
                {userName
                  ? <span style={{ fontSize: 13, fontWeight: 900, color: '#F0EBE0' }}>{userName.charAt(0).toUpperCase()}</span>
      ...
      <AnimatePresence>
        {mobileSearchOpen && (
          <motion.div className="lg:hidden border-t border-white/10"
            style={{ background: 'rgba(248,245,238,0.98)' }}
              ...
              <div className="flex gap-2 pt-1">
                ...
                {userName ? (
                  <a href="/mi-cuenta"
                    ...
                    style={{ background: 'linear-gradient(135deg,#D4AF37,#F0C030)', color: '#F0EBE0' }}>
                    👤 Mi cuenta
                  </a>
                ) : (
                  <a href="/login"

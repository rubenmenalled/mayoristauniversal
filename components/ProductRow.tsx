...
function Card({ p }: { p: Prod }) {
  ...
  const esU = (p.category ?? '').toUpperCase() === 'COTILLON MAGIC KINGDOM' || (p.subcategory ?? '').toUpperCase() === 'POP IT' || esDocena || (['IMPORTADORA MAX','IMPORTADORA COMEX','IMPORTADORA TRENDY'].includes((p.category ?? '').toUpperCase()))
          ...
          {isPack ? (
            <>
              ...
              <div style={{ color: '#C2410C', fontSize: 12, fontWeight: 700, marginTop: 1 }}>
                {esDocena ? `el bulto (${p.minOrder} docenas)` : p.minOrder === 12 ? 'la docena' : `pack x${p.minOrder}`}: {fmt(big)}
              </div>
            </>
          ) : (

...
function Card({ p }: { p: Prod }) {
  ...
  const esU = (p.category ?? '').toUpperCase() === 'COTILLON MAGIC KINGDOM' || (p.subcategory ?? '').toUpperCase() === 'POP IT' || esDocena || (['IMPORTADORA MAX','IMPORTADORA COMEX'].includes((p.category ?? '').toUpperCase()))

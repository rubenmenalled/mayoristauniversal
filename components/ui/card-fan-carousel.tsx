...
export default function SocialCards({ cards }: SocialCardsProps) {
  ...
  return (
    <section className="flex flex-col items-center w-full py-4 lg:py-8 px-4 md:px-8 relative z-20">
      <style>{`
        .fan-layout { position: relative; height: 38rem; }
        .fan-card {
          position: absolute; left: 50%; top: 50%;
          width: 17rem; height: 24rem;
          margin-left: -8.5rem; margin-top: -12rem;
          border-radius: 18px; overflow: hidden;
          background: #0B1E3F;
          box-shadow: 0 18px 50px rgba(11,30,63,0.35);
          will-change: transform;
        }
        @media (max-width: 1023px) {
          .fan-layout { height: 34rem; }
          .fan-card { width: 15rem; height: 21rem; margin-left: -7.5rem; margin-top: -10.5rem; }
        }
        @media (max-width: 767px) {
          .fan-layout { height: 28rem; }
          .fan-card { width: 12.5rem; height: 17.5rem; margin-left: -6.25rem; margin-top: -8.75rem; }
        }
        @media (max-width: 639px) {
          .fan-layout { height: 26rem; }
          .fan-card { width: 11rem; height: 15.5rem; margin-left: -5.5rem; margin-top: -7.75rem; }
        }
        @media (max-width: 479px) {
          .fan-layout { height: 22rem; }
          .fan-card { width: 9.5rem; height: 13.5rem; margin-left: -4.75rem; margin-top: -6.75rem; }
        }
      `}</style>
      ...
      {needsPagination && (
        <div className="flex items-center justify-center gap-4 mt-4 md:mt-6 z-30">
          <button className={`${ARROW_CLASSES} w-10 h-10 md:w-12 md:h-12`} onClick={() => cycle("left")} aria-label="Anterior">
            {chevron("left")}
          </button>
          ...
          <button className={`${ARROW_CLASSES} w-10 h-10 md:w-12 md:h-12`} onClick={() => cycle("right")} aria-label="Siguiente">
            {chevron("right")}
          </button>
        </div>
      )}
